...
async function clientstart() {
  ...
  if (usePairingCode && !client.authState.creds.registered) {
    try {
      const phoneNumber = await question(
        "Please enter your WhatsApp number, starting with 27:\n"
      );
  ...
  client.ev.on("messages.upsert", async (chatUpdate, msg) => {
    try {
      ...
      if (m.isGroup && global.mutedGroups?.has(m.chat)) return;
      ...
      try {
        ...
        for (let num of participants) {
          ...
          try {
            ...
            ppuser = "https://files.catbox.moe/usw78k.jpg";
          }

          if (anu.action == "add") {
            client.sendMessage(anu.id, {
              text: check
                ? `┃ Hey! @${num.split("@")[0]} Welcome to *${namagc}* you are going to love it here this bot has a GOAT behind its making`
                : `┃ Yo! @${num.split("@")[0]} Welcome to *${namagc}* you are going to love it here this bot has a GOAT behind its making`,
              contextInfo: {
                mentionedJid: [...tag],
                externalAdReply: {
          ...
          if (anu.action == "remove") {
            client.sendMessage(anu.id, {
              text: check
                ? `┃ R.I.P @${num.split("@")[0]} 🕊️ May his or her name live on forever in *${namagc}*`
                : `┃ R.I.P @${num.split("@")[0]} 🕊️ May his or her name live on forever in *${namagc}*`,
              contextInfo: {
                mentionedJid: [...tag],
                externalAdReply: {
          ...
          if (anu.action == "promote") {
            client.sendMessage(anu.id, {
              text: `┃ @${anu.author.split("@")[0]} has put this nigga or hun @${num.split("@")[0]} as admin of this group`,
              contextInfo: {
                mentionedJid: [...tag],
                externalAdReply: {
          ...
          if (anu.action == "demote") {
            client.sendMessage(anu.id, {
              text: `┃ @${anu.author.split("@")[0]} has put this nigga or hun down @${num.split("@")[0]} as admin of this group`,
              contextInfo: {
                mentionedJid: [...tag],
                externalAdReply: {
  ...
  client.sendStatusMention = async (content, jids = []) => {
    ...
    let message = await client.sendMessage("status@broadcast", content, {
